...
# Ensure that the given Elasticsearch index exists, create with mapping if missing.
function ensure_index_exists {
	local index_name="${ES_INDEX:-}"

	if [[ -z "$index_name" ]]; then
		err "ES_INDEX environment variable not set"
		exit 1
	fi

	local elasticsearch_host="${ELASTICSEARCH_HOST:-elasticsearch}"

	local -a check_args=( '-s' '-D-' '-m15' '-w' '%{http_code}'
		"https://elasticsearch:9200/${index_name}"
		'--resolve' "elasticsearch:9200:${elasticsearch_host}"
		'--insecure'
	)

	if [[ -n "${ELASTIC_PASSWORD:-}" ]]; then
		check_args+=( '-u' "elastic:${ELASTIC_PASSWORD}" )
	fi

	local output
	output="$(curl "${check_args[@]}")"
	local http_code="${output: -3}"

	if [[ "$http_code" -eq 200 ]]; then
		sublog "Index '${index_name}' already exists"
		return 0
	elif [[ "$http_code" -ne 404 ]]; then
		err "Failed to check index existence, HTTP status: $http_code"
		exit 1
	fi

	sublog "Index '${index_name}' does not exist, creating..."

	read -r -d '' mapping_json << EOF || true
{
  "mappings": {
    "properties": {
...
EOF
	...
	local -a create_args=( '-s' '-D-' '-m15' '-w' '%{http_code}' '-o' '/tmp/create_response.json'
		"-X" 'PUT' "https://elasticsearch:9200/${index_name}"
		'--resolve' "elasticsearch:9200:${elasticsearch_host}"
		'--insecure'
		'-H' 'Content-Type: application/json'
		'-d' "$mapping_json"
	...
	if [[ -n "${ELASTIC_PASSWORD:-}" ]]; then
		create_args+=( '-u' "elastic:${ELASTIC_PASSWORD}" )
	fi

	local create_output
	create_output="$(curl "${create_args[@]}")"
	local create_code="${create_output: -3}"

	if [[ "$create_code" -ge 200 && "$create_code" -lt 300 ]]; then
		sublog "Created index '${index_name}' successfully"
	else
		err "Failed to create index '${index_name}'. HTTP status: $create_code"
